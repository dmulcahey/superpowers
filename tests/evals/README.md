...
The routing gate intentionally starts after the first-turn bypass decision has already been resolved to `enabled` for the synthetic scenario session. The bypass prompt and session-decision contract are covered separately by `bash tests/codex-runtime/test-using-superpowers-bypass.sh`.
