...
If a fresh review has not already been resolved for the current diff, invoke `superpowers:requesting-code-review` before presenting completion options.
...
If a branch-specific test-plan artifact exists and it names pages, routes, or browser interactions, or the current change is clearly browser-facing, require the existing QA handoff before completion options.
