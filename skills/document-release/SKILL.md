...
- If the user explicitly approves the protected-branch release write, run:
...
- If the release-doc pass also commits on the same protected branch, re-run the gate for the commit itself with `--write-target git-commit` before `git commit`.
