...
**Architecture:** Implement the work in four coherent slices. First, centralize repo/branch identity and `_BRANCH` grounding in shared helper/generator surfaces. Second, broaden skill descriptions only in template/frontmatter plus router-safety tests and evals. Third, modernize `superpowers-update-check` cache freshness while preserving Superpowers-specific semver behavior. Fourth, regenerate generated docs, update release notes, and run the full verification matrix.

**Tech Stack:** POSIX shell helpers, PowerShell wrappers, Node-based doc generation/tests, shell regression tests, OpenAI routing evals, Markdown skill docs
...
- `tests/evals/using-superpowers-routing.eval.mjs` already provides a minimal routing-fails-closed eval entrypoint.
...
- Modify: `tests/evals/using-superpowers-routing.eval.mjs`
...
- Before final review, use `superpowers:verification-before-completion` and keep the required Item 1 routing eval as a hard gate, not a nice-to-have.
...
| --- | --- | --- |
| late-stage descriptions | routing drift into later workflow stages | shared contract tests + required routing eval |
...
- Test: `EVALS=1 OPENAI_API_KEY=... node --test tests/evals/using-superpowers-routing.eval.mjs`

- [ ] **Step 1: Add the red contract checks for allowed and forbidden description broadening**
...
- [ ] **Step 2: Expand the routing eval prompt set before changing any descriptions**
...
- [ ] **Step 3: Run the red routing-safety tests**
...
- [ ] **Step 4: Rewrite descriptions in templates by class, not ad hoc**
...
- [ ] **Step 5: Regenerate the skill docs**
...
- [ ] **Step 6: Run deterministic routing-safety tests**
...
- [ ] **Step 7: Run the required focused routing eval for Item 1**

Run: `EVALS=1 OPENAI_API_KEY=... node --test tests/evals/using-superpowers-routing.eval.mjs`
Expected: PASS with explicit evidence that later-sounding prompts still route to the earlier safe stage when helper state demands it.

- [ ] **Step 8: Commit the description-alignment slice**
...
git add \
  ...
  tests/evals/using-superpowers-routing.eval.mjs
...
- [ ] **Step 1: Add red coverage for split TTLs and forced cache busting**
...
- [ ] **Step 2: Run the red update-check suite**
...
- [ ] **Step 3: Implement `--force` and split cache freshness without changing semver truth**
...
- [ ] **Step 4: Confirm wrapper passthrough still works for the new CLI flag**
...
- [ ] **Step 1: Add release-note coverage for the shipped runtime behavior changes**
...
- [ ] **Step 2: Run a final regeneration pass**

Run: `node scripts/gen-skill-docs.mjs`
Expected: no manual edits remain in generated `SKILL.md` outputs.

- [ ] **Step 3: Verify generated outputs are clean**

Run: `node scripts/gen-skill-docs.mjs --check`
Expected: PASS

- [ ] **Step 4: Run the deterministic verification matrix**
...
- [ ] **Step 5: Run the required Item 1 eval one last time on the combined diff**
...
git add tests/codex-runtime/*.sh tests/codex-runtime/*.mjs tests/evals/using-superpowers-routing.eval.mjs
git commit -m "docs: record borrowed-layer alignment changes"
```

## Ready-For-Review Checklist

- [ ] `docs/superpowers/specs/2026-03-18-gstack-borrowed-layer-alignment-design.md` is re-approved as `CEO Approved` with `Spec Revision: 2`
- [ ] `docs/superpowers/plans/2026-03-19-gstack-borrowed-layer-alignment.md` stays `Draft` with `Execution Mode: none` until engineering review resolves
...
- [ ] Item 1 required routing eval passes
