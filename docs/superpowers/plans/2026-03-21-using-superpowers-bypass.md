...
- [ ] **Step 3: Implement the dedicated generator path**
```js
...
Add whatever minimal script/test helpers are needed so the shell regression can inspect the generated `using-superpowers` contract and emulate the decision-file state transitions deterministically without introducing a new helper binary.
